...
<html lang="fr">
	<head>
		...
		<link rel="stylesheet" href="K-PROJECT/css/style.css">
		<script src='https://cdnjs.cloudflare.com/ajax/libs/jquery/3.2.1/jquery.min.js'></script>
	...
	<body >			
			<!-- inclusion et génération d'une liste à puce de tous les enseignants à partir 
				 de la fonction afficherEnseignants du fichier affichage_objets.inc.php-->
			<div class="top-dish-area section-gap">
				<?php
					include_once(DOSSIER_BASE_INCLUDE."vues/inclusions/affichage_erreurs.inc.php");
					include_once(DOSSIER_BASE_INCLUDE."vues/inclusions/affichage_confirmation.inc.php");
					if (count($controleur->getMessagesErreur()) != 0) {
						afficherListeErreurs($controleur->getMessagesErreur());
					}
					else if(count($controleur->getMessagesConfirmation()) != 0){
						afficherListeConfirmation($controleur->getMessagesConfirmation());
					}
					echo "<h1>Page d'inscription pour les utilisateurs</h1>";
					include_once(DOSSIER_BASE_INCLUDE.'vues/inclusions/formulaireInscription.inc.php');
				?>
			</div>
	</body>
</html>
